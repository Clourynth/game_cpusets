...
# Fungsi untuk memeriksa konfigurasi CPU cluster (2+6)
check_cpu_cluster() {
    ...
    if [ "$CLUSTER_0_COUNT" -eq 6 ] && [ "$CLUSTER_1_COUNT" -eq 2 ] && \
       [ "$CLUSTER_2_COUNT" -eq 0 ] && [ "$CLUSTER_3_COUNT" -eq 0 ]; then
        ui_print "- Detected 2+6 CPU cluster configuration."
        return 0
    fi

    ui_print "! Unsupported CPU configuration."
    ui_print "! This module requires a 2+6 CPU cluster."
    return 1
}
